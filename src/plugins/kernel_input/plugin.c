...
void kernel_event_status_handle (KernelEventStatus* self, struct input_event* ev) {
	...
	switch (_tmp4_) {
		case KERNEL_KEY_PRESS:
		{
			GTimeVal _tmp5_ = {0};
			gboolean _tmp6_;
			KernelAggregateInputDevice* _tmp8_;
			const gchar* _tmp9_;
			g_get_current_time (&_tmp5_);
			self->priv->timestamp = _tmp5_;
			self->priv->pressed = TRUE;
			_tmp6_ = self->priv->reportheld;
			if (_tmp6_) {
				guint _tmp7_ = 0U;
				_tmp7_ = g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 1050, _kernel_event_status_onTimeout_gsource_func, kernel_event_status_ref (self), kernel_event_status_unref);
				self->priv->timeout = _tmp7_;
			}
			_tmp8_ = aggregate;
			_tmp9_ = self->priv->name;
			g_signal_emit_by_name ((FreeSmartphoneDeviceInput*) _tmp8_, "event", _tmp9_, FREE_SMARTPHONE_DEVICE_INPUT_STATE_PRESSED, 0);
			break;
		}
		case KERNEL_KEY_RELEASE:
		{
			gboolean _tmp10_;
			_tmp10_ = self->priv->pressed;
			if (!_tmp10_) {
				KernelAggregateInputDevice* _tmp11_;
				FsoFrameworkLogger* _tmp12_;
				KernelAggregateInputDevice* _tmp13_;
				const gchar* _tmp14_;
				_tmp11_ = aggregate;
				_tmp12_ = ((FsoFrameworkAbstractObject*) _tmp11_)->logger;
				fso_framework_logger_warning (_tmp12_, "Received release event before pressed event!?");
				_tmp13_ = aggregate;
				_tmp14_ = self->priv->name;
				g_signal_emit_by_name ((FreeSmartphoneDeviceInput*) _tmp13_, "event", _tmp14_, FREE_SMARTPHONE_DEVICE_INPUT_STATE_RELEASED, 0);
			} else {
				guint _tmp15_;
				KernelAggregateInputDevice* _tmp17_;
				const gchar* _tmp18_;
				guint _tmp19_ = 0U;
				self->priv->pressed = FALSE;
				...
				if (_tmp15_ > ((guint) 0)) {
					guint _tmp16_;
					_tmp16_ = self->priv->timeout;
					g_source_remove (_tmp16_);
				}
				_tmp17_ = aggregate;
				_tmp18_ = self->priv->name;
				_tmp19_ = kernel_event_status_age (self);
				g_signal_emit_by_name ((FreeSmartphoneDeviceInput*) _tmp17_, "event", _tmp18_, FREE_SMARTPHONE_DEVICE_INPUT_STATE_RELEASED, (gint) _tmp19_);
			}
			break;
		}
