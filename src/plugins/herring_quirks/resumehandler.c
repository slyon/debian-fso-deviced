/* resumehandler.c generated by valac 0.14.2, the Vala compiler
...
static gboolean _____lambda0_ (HerringResumeHandler* self);
static gboolean ______lambda0__gsource_func (gpointer self);
...
static gboolean __lambda1_ (HerringResumeHandler* self);
	...
	if ((_tmp1_ & G_IO_IN) == G_IO_IN) {
		...
		if (_tmp4_ == ((gssize) 0)) {
			...
			if (((gint) _tmp13_) == _tmp14_) {
				...
				g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, ______lambda0__gsource_func, g_object_ref (self), g_object_unref);
			} else {
...
HerringResumeHandler* herring_resume_handler_construct (GType object_type) {
	...
	g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, ___lambda1__gsource_func, g_object_ref (self), g_object_unref);
	_tmp0_ = ((FsoFrameworkAbstractObject*) self)->config;
	_tmp1_ = fso_framework_smart_key_file_intValue (_tmp0_, HERRING_MODULE_NAME, "wakeup_inputnode", -1);
	self->priv->inputnodenumber = _tmp1_;
	_tmp2_ = ((FsoFrameworkAbstractObject*) self)->config;
	_tmp3_ = fso_framework_smart_key_file_intValue (_tmp2_, HERRING_MODULE_NAME, "wakeup_powerkeycode", -1);
	self->priv->powerkeycode = _tmp3_;
	return self;
}
