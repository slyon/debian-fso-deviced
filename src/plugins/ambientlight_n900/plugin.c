...
extern AmbientLightN900* instance;
AmbientLightN900* instance = NULL;
...
gchar* fso_factory_function (FsoFrameworkSubsystem* subsystem, GError** error) {
	...
	sysfs_root = _tmp3_;
	_tmp4_ = sysfs_root;
	_tmp5_ = g_build_filename (_tmp4_, AMBIENT_LIGHT_DEFAULT_NODE, NULL);
	dirname = _tmp5_;
	_tmp6_ = dirname;
	_tmp7_ = string_to_string (_tmp6_);
	_tmp8_ = g_strconcat ("looking in ", _tmp7_, NULL);
	_tmp9_ = _tmp8_;
	g_debug ("plugin.vala:99: %s", _tmp9_);
	_g_free0 (_tmp9_);
	_tmp10_ = dirname;
	...
	if (_tmp11_) {
		...
		const gchar* _tmp13_;
		AmbientLightN900* _tmp14_;
		_tmp12_ = subsystem;
		_tmp13_ = dirname;
		_tmp14_ = ambient_light_n900_new (_tmp12_, _tmp13_);
		_g_object_unref0 (instance);
		instance = _tmp14_;
	} else {
		FsoFrameworkLogger* _tmp15_;
		_tmp15_ = fso_framework_theLogger;
		fso_framework_logger_error (_tmp15_, "No ambient light device found; ambient light object will not be availa" \
"ble");
	}
	_tmp16_ = g_strdup ("fsodevice.ambientlight_n900");
	result = _tmp16_;
	_g_free0 (dirname);
...
void fso_register_function (GTypeModule* module) {
	...
	fso_framework_logger_debug (_tmp0_, "fsodevice.ambientlight_n900 fso_register_function()");
}
